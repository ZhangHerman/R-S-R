...
class Self_Attn(nn.Module):
    ...
    #调用：self.Attention = attention.Self_Attn(2048,8,16,7,7,256)
    #当batchsize为8时，调用Attention时的代码段为：self.Attention = attention.Self_Attn(2048,8,16,7,7,256)
    ...
    def __init__(self,in_dim,batch_size,num_frames,width,height,channels):
        ...
        zero = torch.zeros(2048,256,1,1,1)
        ...
        self.pos_embedV = nn.Parameter(torch.zeros(1,16,256*7*7)) 
    ...
    def forward(self,x):
        ...
        #Q
        q = self.conv1(x) #[8*16,4096,7,7]  [8,512,7,7]  [64,1024,7,7] [64,4096,7,7]
        q = create5D.create5Dimages(q) #[4,16,4096,7,7]
        q = q.view(8,16,256*7*7) #[4,16,4096*7*7]
        q = q+self.pos_embedQ
        q = q.view(8,16,256,7,7)
        ...
        x5 = q[64:80,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height) #[1,16,256,7,7] [1,16,4096,7,7]
        x6 = q[80:96,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)
        x7 = q[96:112,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)
        x8 = q[112:128,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)



        q = torch.cat([x1,x2,x3,x4,x5,x6,x7,x8],dim = 0).permute(0,1,3,4,2) #[8,16,256,7,7]->[4,16,7,7,256]
        ...
        x5 = k[64:80,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height) #[1,16,256,7,7] [1,16,4096,7,7]
        x6 = k[80:96,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)
        x7 = k[96:112,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)
        x8 = k[112:128,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)
        ...
        x5 = v[64:80,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height) #[1,16,256,7,7] [1,16,4096,7,7]
        x6 = v[80:96,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)
        x7 = v[96:112,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)
        x8 = v[112:128,:,:,:].view(1,self.num_frames,self.channels,self.width,self.height)


        v = torch.cat([x1,x2,x3,x4,x5,x6,x7,x8],dim = 0).permute(0,1,3,4,2) #[4,16,256,7,7]->[4,16,7,7,256]
        ...
        out5 = out[4:5,:,:,:,:].view(self.num_frames,self.chanel_in,self.width,self.height) #[1,16,2048,7,7] [1,16,4096,7,7]
        out6 = out[5:6,:,:,:,:].view(self.num_frames,self.chanel_in,self.width,self.height)
        out7 = out[6:7,:,:,:,:].view(self.num_frames,self.chanel_in,self.width,self.height)
        out8 = out[7:8,:,:,:,:].view(self.num_frames,self.chanel_in,self.width,self.height)
        out = torch.cat([out1,out2,out3,out4,out5,out6,out7,out8],dim = 0) #[64,2048,7,7]
