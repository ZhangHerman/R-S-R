...
class TSNDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if('something' or 'hmdb51' in self.dataset):
            decode_boo = False
    ...
    def get(self, record,video_list, indices,decode_boo):
        images = list()
        for seg_ind in indices: # 16
            p = int(seg_ind)
            for i in range(0,self.new_length,1): #self.new_length = 5             
                if(decode_boo):
        ...
        #print(process_data.shape,'........')  #[240,224,224]  
