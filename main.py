...
def main():
    ...
    train_augmentation = model.get_augmentation(
        flip=False if 'something' or 'hmdb51' in args.dataset else True)
...
def train(train_loader, model, criterion, optimizer, epoch, logger=None, scheduler=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        output,scores_rbrf,labels_rbrf,scores_rbrc,labels_rbrc = model(input_var)
        

        #rot loss
        scores_rbrf = scores_rbrf.cuda()
        scores_rbrc = scores_rbrc.cuda()

        labels_rbrf = labels_rbrf.cuda()
        labels_rbrc = labels_rbrc.cuda()

        loss = criterion(output, target_var)
        
        loss_rbrf = criterion(scores_rbrf,labels_rbrf)
        loss_rbrc = criterion(scores_rbrc,labels_rbrc)

        #multi attention loss
        #multi_score = multi_score.cuda()
        #loss_mta = criterion(multi_score,target_var)


        
        #total loss
        loss = 1.0*loss+0.01*loss_rbrf+0.1*loss_rbrc #小数据集0.01,0.005  大数据集：0.1，0.001
        #当然也可以尝试一下其他的取值，比如说0.2,0.3,0.4,0.5,...,...<=1(alpha=1效果非常差)
...
def validate(val_loader, model, criterion, logger=None):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            target = target.cuda()
            output,scores_rbrf,labels_rbrf,scores_rbrc,labels_rbrc = model(input)
